...
import com.hypercell.orange.repository.BucketRepository;
...
/**
 * REST controller for managing {@link com.hypercell.orange.domain.Bucket}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class BucketResource {
    ...
    private static final String ENTITY_NAME = "simMangmentToolBucket";
     ...
     */
    @GetMapping("/buckets")
    public List<Bucket> getAllBuckets(@RequestParam(required = false) String filter) {
        if ("dial-is-null".equals(filter)) {
            log.debug("REST request to get all Buckets where dial is null");
            return StreamSupport
                .stream(bucketRepository.findAll().spliterator(), false)
                .filter(bucket -> bucket.getDial() == null)
